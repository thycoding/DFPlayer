...
//% weight=10 color=#019b9b icon="\uf121" block="DFPlayer"
namespace dfplayermini {
    ...
    /**
     * Connect DFPlayer 
     * @param pinRX RX Pin, eg: SerialPin.P8
     * @param pinTX TX Pin, eg: SerialPin.P12
     */
    //% blockId="dfplayermini_connect" block="connect to DFPlayer, RX:%pinRX|TX:%pinTX"
    ...
    //% blockId="dfplayermini_press" block="execute :%myPlayType"
    //% weight=99 blockGap=20
    export function press(myPlayType: playType): void {
        innerCall(myPlayType, 0x00, 0x00)
    }

    //% blockId="dfplayermini_playFile" block="play DFPlayer the file number:%fileNumber|repeat:%setRepeat"
    ...
    //% blockId="dfplayermini_playFileInFolder" block="play DFPlayer in the folder number:%folderNumber|file number:%fileNumber|repeat:%setRepeat"
